...
from common.utils.network_filter import NetworkFilter
from common.network_error_monitor import NetworkErrorMonitor
...
class BrowserCoreUC:
    ...
    def __init__(self, instance_id: int = 1, worker_id: str = None, vpn_interface: str = None):
        """
        Args:
            instance_id: 인스턴스 ID
            worker_id: 워커 식별자 (예: "Worker-1") - 네트워크 에러 로깅용
            vpn_interface: VPN 인터페이스 (예: "wg101") - 네트워크 에러 로깅용
        """
        self.instance_id = instance_id
        self.worker_id = worker_id or f"Instance-{instance_id}"
        ...
        # Network error monitor (ERR_NETWORK_CHANGED 감지)
        self.network_error_monitor = None
    ...
    def launch(
        ...
        # 버전별 프로필 사용 (사용자 자체가 이미 wg101, wg102로 분리되어 있음)
        ...
        if chromedriver_bin.exists():
            ...
            # 사용자별 + 버전별 분리 (wg101, wg102 등 사용자 자체가 이미 분리됨)
            instance_driver_dir = Path(f"/tmp/chromedriver_{current_user}_v{version}")
            ...
            if not instance_driver_path.exists():
                ...
                print(f"   📋 ChromeDriver 복사: {current_user} 전용")
        ...
        # 브라우저 시작 이벤트 로깅
        print(f"   📊 Unified logger available: {UNIFIED_LOGGER_AVAILABLE}")
        if UNIFIED_LOGGER_AVAILABLE:
            try:
                print(f"   📝 Logging BROWSER_LAUNCHING (worker={self.worker_id}, interface={self.vpn_interface})")
                log_event(
                    worker_id=self.worker_id,
                    event_type=EventType.BROWSER_LAUNCHING,
                    interface=self.vpn_interface,
                    details={'version': version_main, 'headless': headless}
                )
                print(f"   ✅ BROWSER_LAUNCHING logged")
            except Exception as e:
                print(f"   ❌ Failed to log BROWSER_LAUNCHING: {e}")
        ...
        if UNIFIED_LOGGER_AVAILABLE:
            log_event(
                ...
                interface=self.vpn_interface,
                details={'version': version_main}
            )

        # 네트워크 에러 모니터 시작 (ERR_NETWORK_CHANGED 감지)
        if Config.ENABLE_NETWORK_ERROR_MONITOR:
            try:
                self.network_error_monitor = NetworkErrorMonitor(
                    self.driver,
                    worker_id=self.worker_id,
                    interface=self.vpn_interface
                )
                self.network_error_monitor.start(interval=0.5)
                print(f"   ✓ 네트워크 에러 모니터 시작 (ERR_NETWORK_CHANGED 감지)")
            except Exception as e:
    ...
    def close_browser(self):
        ...
        # 네트워크 에러 모니터 중지
        if self.network_error_monitor:
            try:
                self.network_error_monitor.stop()
                print(f"   ✓ 네트워크 에러 모니터 중지")
            except Exception as e:
                print(f"   ⚠️  네트워크 에러 모니터 중지 실패: {e}")

        # 브라우저 종료 전 캐시 정리 (핑거프린팅 보존)
        try:
            self.cleanup_profile_on_exit()
        except Exception as e:
            print(f"   ⚠️  프로필 정리 실패: {e}")
    ...
    def cleanup_profile_on_exit(self):
        ...
        for dir_name in root_dirs:
            dir_path = profile_root / dir_name
            if dir_path.exists() and dir_path.is_dir():
                try:
                    dir_size = sum(f.stat().st_size for f in dir_path.rglob('*') if f.is_file())
                    shutil.rmtree(dir_path)
        ...
        for pattern in fingerprint_files:
            for file_path in default_dir.glob(pattern):
        ...
        if deleted_count > 0:
            print(f"   ✅ {deleted_count}개 항목 정리 완료 ({deleted_size / 1024 / 1024:.1f}MB)")
            print(f"   ✅ 시크릿 모드 효과: 쿠키, 히스토리, 핑거프린팅 데이터 삭제")
            print(f"   💾 HTTP 캐시 보존: jQuery, Bootstrap 등 재사용 (트래픽 32% 절감)")
        else:
            print(f"   ℹ️  정리할 항목 없음")
    ...
    def take_screenshot(
        ...
        """
        from uc_lib.modules.screenshot_capturer import ScreenshotCapturer
